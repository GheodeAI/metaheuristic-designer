...
class ScipyWrapper:
    ...
    def optimize(self):
        ...
        bounds = np.atleast_2d(np.asarray([self.objfunc.lower_bound, self.objfunc.upper_bound]).T)
        ...
        if self.method == "differential_evolution":
            res = differential_evolution(self.objfunc.objective, bounds, maxiter=self.maxiter, callback=callback, polish=False, **self.opt_kwargs)
            ...
            res = basinhopping(self.objfunc.objective, x0, callback=callback, polish=False, **kwargs)
        elif self.method == "dual_annealing":
            res = dual_annealing(self.objfunc.objective, bounds, maxiter=self.maxiter, callback=callback, polish=False, **self.opt_kwargs)
        else:
            raise ValueError(f"Unknown SciPy method: {self.method}")

        self.best_obj = res.fun
