...
def particle_swarm(objfunc, params):
    encoding_str = params["encoding"] if "encoding" in params else "real"

    if encoding_str.lower() == "real":
        alg = particle_swarm_real_vec(objfunc, params)
    elif encoding_str.lower() == "int":
        alg = particle_swarm_int_vec(objfunc, params)
    elif encoding_str.lower() == "bin":
        alg = particle_swarm_bin_vec(objfunc, params)
    else:
...
def particle_swarm_real_vec(objfunc, params):
    """
...
def particle_swarm_int_vec(objfunc, params):
    """
    Instantiates a particle swarm algorithm to optimize the given objective function.
    This objective function should accept real coded vectors.
    """
...
def particle_swarm_bin_vec(objfunc, params):
    """
    Instantiates a particle swarm algorithm to optimize the given objective function.
    This objective function should accept real coded vectors.
    """
