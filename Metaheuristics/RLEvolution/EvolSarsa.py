...
class EvolSarsa:
    ...
    def evolve(self):
        ...
        if random.random() < self.eps:
            self.curr_action = random.randrange(len(self.actions))
        else:
            self.curr_action = self.q_table[self.curr_state, :].argmax()
         
        # choose individual to apply the operator to
        fitness_list = [i.get_fitness() for i in self.population]
    ...
    def replace(self):
        ...
        new_fitness = self.new_ind.get_fitness()
    ...
    def update_table(self, reward):
        ...
        s_1 = self.curr_state
        a_1 = self.curr_action

        # use the SARSA update rule for the q-table
        self.q_table[s_0, a_0] += self.alpha * (reward + self.discount * self.q_table[s_1, a_1] - self.q_table[s_0, a_0])
