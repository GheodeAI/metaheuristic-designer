...
ioh = pytest.importorskip(
    "ioh", reason="IOHexperimenter not installed (optional dependency)"
)
...
def test_name_contains_identifiers(problem_sphere):
    assert "IOH-bbob-F1-D2-ins1" in problem_sphere.name


# ----- Evaluation --------------------------------------------------------

def test_optimum_near_known_value(problem_sphere):
    opt_x = problem_sphere._ioh_problem.optimum.x
    opt_y = problem_sphere._ioh_problem.optimum.y
    val = problem_sphere.objective(np.array(opt_x))
    assert np.isclose(val, opt_y, atol=1e-4), \
        f"Expected {opt_y}, got {val}"
