...
def test_gauss_operator_modifies_genotype(rng, dummy_objfunc, simple_encoding):
    ...
    op = create_mutation_operator("gauss", encoding=simple_encoding, random_state=rng, N=2, distrib="gauss", loc=0, scale=1)
    result = op(pop)
...
def test_xor_operator_on_zeros_population(rng, dummy_objfunc, simple_encoding):
    ...
    op = create_mutation_operator("xor", encoding=simple_encoding, random_state=rng, N=2, BinRep="byte")
    result = op(pop)
...
def test_mutation_operator_reproducible(rng, dummy_objfunc, simple_encoding):
    ...
    op1 = create_mutation_operator("gauss", encoding=simple_encoding, random_state=rng1, N=2, distrib="gauss", loc=0, scale=1)
    op2 = create_mutation_operator("gauss", encoding=simple_encoding, random_state=rng2, N=2, distrib="gauss", loc=0, scale=1)
